...
export default function NewPostForm({ onPost }: { onPost: (data: { author?: string; content: string; image?: string | null }) => void }) {
  const input = useCurrent<{ author: string; content: string; image: string | null }>({ author: '', content: '', image: null });

  const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    const f = e.target.files && e.target.files[0];
    if (!f) return;
    const r = new FileReader();
    r.onload = ev => input.value.image = ev.target?.result as string;
    r.readAsDataURL(f);
  };
